...
class DeepseekCompressor(nn.Module):
    ...
    def forward(
        ...
        kv_score = self.fused_wkv_wgate(x)
        ...
        # Add APE to score
        # [num_tokens, coff * self.head_dim]
        score = score + self.ape[positions % self.compress_ratio]

        # NOTE: Unlike kv and score, compressed_kv uses bf16 instead of fp32.
        ...
        # Store the KV and score in the state
        ...
        # Compute the compressed KV and RMSNorm later
        _fused_compress_kv_rmsnorm_kernel[(num_actual,)](
            compressed_kv,
        ...
        # Apply RoPE
        compressed_pos = (positions // self.compress_ratio) * self.compress_ratio
        compressed_kv, _ = rotary_emb(compressed_pos, compressed_kv.unsqueeze(1))
        compressed_kv = compressed_kv.squeeze(1)
        return compressed_kv


@triton.jit
def _fused_compress_kv_rmsnorm_kernel(
    compressed_kv_ptr,
    ...
    # Guard against padding tokens during CUDA graph replay.
    # Padding tokens retain stale positions from previous replays which may
    # satisfy the compression-boundary check below, causing the kernel to
    # proceed with invalid block_table entries and crash.
    ...
    # HEAD_SIZE for current window (uses second half),
    # 0 for previous window (uses first half)
    # [(1 + OVERLAP) * COMPRESS_RATIO]
    head_offset = (tokens >= COMPRESS_RATIO).to(tl.int32) * HEAD_SIZE

    block = tl.arange(0, TRITON_BLOCK_SIZE)
    mask = block < HEAD_SIZE

    # Cast block_numbers to int64 to avoid int32 overflow in
    # block_numbers * state_cache_stride0 (can exceed 2^31 when
    # there are >= ~262 K state-cache blocks).
    block_numbers_i64 = block_numbers.to(tl.int64)

    # [(1 + OVERLAP) * COMPRESS_RATIO, TRITON_BLOCK_SIZE]
    score = tl.load(
        state_cache_ptr
    ...
    # Load rms_norm_weight as 1D [TRITON_BLOCK_SIZE]
    rms_norm_weight = tl.load(
        rms_norm_weight_ptr + block,
        mask=mask,
        other=0.0,
    )
    ...
    # RMSNorm: divide by HEAD_SIZE
    variance = tl.sum(compressed_kv * compressed_kv, axis=0) / HEAD_SIZE
    rrms = tl.rsqrt(variance + rms_norm_eps)
    compressed_kv_normed = (compressed_kv * rrms).to(tl.bfloat16) * rms_norm_weight.to(
        tl.bfloat16
    )
