...
from vllm.model_executor.layers.mhc import mhc_post, mhc_pre  # noqa: F401
from vllm.model_executor.layers.rotary_embedding import get_rope
from vllm.model_executor.layers.svf_attention import (
    SVFMLAModules,
    SVFMultiHeadLatentAttentionWrapper,
)
...
class DeepseekSVFMoE(nn.Module):
    def __init__(
        self,
...
class DeepseekSVFAttention(nn.Module):
    def __init__(
        self,
        vllm_config: VllmConfig,
        prefix: str,
        ...
        # Initialize rotary embedding BEFORE SVFMLAModules (which needs it)
        ...
        if self.compress_ratio == 4:
            ...
            self.indexer = DeepseekV32Indexer(
                vllm_config,
                config=config,
        ...
        self.mla_attn = SVFMultiHeadLatentAttentionWrapper(
            hidden_size=self.hidden_size,
...
class DeepseekSVFDecoderLayer(nn.Module):
    def __init__(
        self,
        vllm_config,
        prefix,
        ...
        self.attn = DeepseekSVFAttention(
            vllm_config,
            prefix=f"{prefix}.attn",
            topk_indices_buffer=topk_indices_buffer,
        )
        self.ffn = DeepseekSVFMoE(vllm_config, prefix=f"{prefix}.ffn")
...
class DeepseekSVFModel(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: DeepseekSVFDecoderLayer(
                vllm_config,
                prefix=prefix,
...
class DeepseekSVFForCausalLM(nn.Module):
    model_cls = DeepseekSVFModel
